...
export async function GET(req: Request) {
  ...
  try {
    ...
    const maxWindowDate = new Date(startOfToday.getTime() + maxWindowDays * 24 * 60 * 60 * 1000)

    const bills = await prisma.bill.findMany({
      where: {
        status: { in: ['UNPAID', 'OVERDUE'] },
        dueDate: { lte: maxWindowDate },
      },
      include: {
        category: true,
        user: true,
    ...
    for (const [, userBills] of Object.entries(byUser)) {
      ...
      const userWindowDate = new Date(startOfToday.getTime() + user.reminderDays * 24 * 60 * 60 * 1000)
      const relevantBills = userBills.filter(
        b => b.status === 'OVERDUE' || b.dueDate <= userWindowDate
      )
      if (relevantBills.length === 0) continue

      const { html, text } = billReminderEmail({
        userName: user.name ?? user.email.split('@')[0],
        bills: relevantBills.map(b => ({
          title: b.title,
          amount: b.amount,
          dueDate: b.dueDate,
      ...
      const overdueCount = relevantBills.filter(b => b.status === 'OVERDUE').length
      const subjectUrgency = overdueCount > 0
        ? `⚠️ ${overdueCount} overdue bill${overdueCount > 1 ? 's' : ''}`
        : `⏰ ${relevantBills.length} bill${relevantBills.length > 1 ? 's' : ''} due soon`
      ...
      results.push({
        email: user.email,
        billCount: relevantBills.length,
